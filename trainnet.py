...
if __name__ == '__main__':
    ...
    for epoch in range(args.start_epoch, args.max_epochs + 1):
        ...
        if args.mGPUs:
            ...
            if cfg.POOLING_MODE == 'crop':
                save_name = os.path.join(output_dir,
                                         'faster_rcnn_{}_{}_{}_ROIALIGN_SAMPLE{}.pth'.format(args.session, epoch, step,
                                                                                             cfg.ALIGN_SAMPLE_NUM * cfg.ALIGN_SAMPLE_NUM))
            elif cfg.POOLING_MODE == 'pool':
                save_name = os.path.join(output_dir,
                                         'faster_rcnn_{}_{}_{}_ROIPOOLING.pth'.format(args.session, epoch, step))
            elif cfg.POOLING_MODE == 'align':
                save_name = os.path.join(output_dir,
                                         'faster_rcnn_{}_{}_{}_ORIGINALALIGN.pth'.format(args.session, epoch, step))
        ...
        if args.poolingmode == 'pool':
            label = "pool"
        elif args.poolingmode == 'crop':
            label = "align with {} sample".format(cfg.ALIGN_SAMPLE_NUM * cfg.ALIGN_SAMPLE_NUM)

        np.save('{}/{}_jointloss.npy'.format(output_dir, label), jointloss)
        np.save('{}/{}_rpnclsloss.npy'.format(output_dir, label), rpnclsloss)
        np.save('{}/{}_rpnboxloss.npy'.format(output_dir, label), rpnboxloss)
        np.save('{}/{}_rcnnclsloss.npy'.format(output_dir, label), rcnnclsloss)
        np.save('{}/{}_rcnnboxloss.npy'.format(output_dir, label), rcnnboxloss)
